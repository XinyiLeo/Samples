...
int main(int argc, char* argv[]) {
	...
	FILE* pFileR = fopen("EulerR.out" , "w"); 
	FILE* pFileU = fopen("EulerU.out" , "w"); 
	FILE* pFileV = fopen("EulerV.out" , "w"); 
	FILE* pFileP = fopen("EulerP.out" , "w"); 
	...
	double* swap = (double*)malloc(N*N*sizeof(double));
	...
	for(int loop = 0; loop < M; loop++) {
		...
		// Before update p, use swap temporary grid to store original p
		for(int i = 0; i < N*N; i++) {
			 swap[i] = p[i];
		}
		...
		for(int x = 0; x < N; x++) {     // Transpose here
			for(int y = 0; y < N; y++) {
				b[x*4*N+y] = rho[y*N+x];
				b[x*4*N+N+y] = u[y*N+x];
				b[x*4*N+2*N+y] = v[y*N+x];
				b[x*4*N+3*N+y] = p[y*N+x];
			}
		}
	...
	// Write the result 
	for(int i = 0; i < N; i++) {
		fwrite(rho, sizeof(double), N, pFileR);
	}
	
	for(int i = 0; i < N; i++) {
		fwrite(u, sizeof(double), N, pFileU);
	}

	for(int i = 0; i < N; i++) {
		fwrite(v, sizeof(double), N, pFileV);
	}

	for(int i = 0; i < N; i++) {
		fwrite(p, sizeof(double), N, pFileP);
	}
	fclose(pFileR);
	fclose(pFileU);
	fclose(pFileV);
