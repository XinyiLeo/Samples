...
int main(int argc, char *argv[])
{
	...
	priority_queue<point_t*, compare_t> *pql = new unsorted_heap<point_t*, compare_t>;
...
//#define SEE_QUEUE
		...
		if (ifValid(a, C_temp->x, C_temp->y, RIGHT, grid.width, grid.height))
		{
			...
			{
    				cout << "Cell (" << N_temp->x << ", " << N_temp->y << ") with accumulated length " 
		                     << N_temp->pathcost << " is the ending point." << endl;
			...
			{
   				cout << "Cell (" << N_temp->x << ", " << N_temp->y << ") with accumulated length " 
		                     << N_temp->pathcost << " is added into the queue." << endl;
				pql->enqueue(N_temp);
			}
		}
